...
That command IS the session-start protocol (`/startup` in Codex). It is read-only and prints the environment, this session's number, git state, the CURRENT STATE block, one line per open loop, the content gate, the house rules and any flags. Read all of it before replying to Hugo. Then:
...
Hugo works on this folder from **Codex** (terminal on his Mac: full shell, background jobs survive, local fonts, rm works) and **Cowork** (Codex desktop app: sandboxed Linux, 45s per call, no background jobs, rm blocked in the mount), often alternating within a day. Paths under `/sessions/<name>/mnt/hyperframes/` mean Cowork; `/Users/hugobrizuela/...` means Codex. Environment and tool gotchas live in `docs/gotchas.md`, which startup names but does not print; read it when a tool misbehaves and add new learnings there, labelled by environment.
...
- **Generated media** goes in `clients/<client>/generated/images/` or `generated/videos/`, and every keeper's prompt is saved to the client's `image-prompts.md` (the prompt is the source of truth, binaries are gitignored). Iterate at quality low in Cowork, render finals in Codex.
