#!/usr/bin/env python3
"""Email 02 social batch, v4. v3 placement plus the Sportif house grade (D-041).

Same lockup positions as v3. The only change is that every photo now carries the
house peach tone at 0.45, applied to the PHOTO before any type goes on, so the
black wordmark stays true black. See scripts-local/sportif_grade.py.
...
from sportif_grade import grade_image, HOUSE_STRENGTH
...
OUT = ROOT / 'clients/sportif/email-02-social/created/v4'
...
def build(src, fmt, size, treat, name):
    W, H = size
    # house grade goes on the PHOTO, before the mark, so the type stays true black
    img = grade_image(cover(src, W, H), HOUSE_STRENGTH).convert('RGBA')
